        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-10 space-y-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-2">
              <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Production Plan List</h1>
              <p className="text-slate-600">Danh sách kế hoạch sản xuất theo dõi tiến độ.</p>
            </div>
            <Link
            ...
            <div className="overflow-x-auto">
              <table className="min-w-275 w-full divide-y divide-slate-200 table-auto">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="w-28 px-5 py-3 text-left text-sm font-semibold text-slate-700">Kế hoạch</th>
                    <th className="w-28 px-4 py-3 text-left text-sm font-semibold text-slate-700">Production</th>
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {pageData.length === 0 ? (
                    ...
                    pageData.map((item) => (
                      <tr key={item.planId} className="hover:bg-slate-50 transition-colors">
                        <td className="px-5 py-3 text-sm text-slate-600 font-medium">#PL-{item.planId}</td>
                        ...
                        <td className="px-4 py-3 text-right">
                          <Link to={`/monitoring/${item.planId}`} className="text-emerald-600 hover:text-emerald-800 text-sm font-medium">
                            Chi tiết →
                          </Link>
                        </td>
                      </tr>
