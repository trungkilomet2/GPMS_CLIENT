...
export default function WorkerCuttingBook() {
  ...
  const handleCreate = async () => {
    ...
    try {
      setIsCreating(true);
      await CuttingNotebookService.createNotebook({
        productionId: Number(locationProductionId),
        fabricWidth: Number(newBook.fabricWidth),
        markerLength: Number(newBook.markerLength),
      });
      toast.success("Tạo sổ cắt thành công.");
      setShowCreateModal(false);
      setNewBook({ fabricWidth: "", markerLength: "" });
      fetchNotebooks();
    } catch (err) {
      console.error("Lỗi tạo sổ cắt:", err);
